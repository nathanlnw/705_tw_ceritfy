...
//------  voice -----
u8    Flag_tired=1;        //  tired start 1   开始
u32  TiredStart_time=0;
u32  TiredEnalbTime=0;
u32  Tired_ClearStart=0;
u32  Tired_ClearEnable=0;   
...
u8      Jia_GPS_spdFlag=0;  //  假gps spd Flag
u16     Speed_gps_jiashi=0;   //  假时GPS 真速度
//-----  车台注册定时器  ----------
...
{  	
  ...
  u8 i=0;
		  ...
		  //  JT808Conf_struct.Regsiter_Status=1; //标注注册，但不存储	 	  
            ...
            if((1==JT808Conf_struct.DURATION.Heart_SDFlag)&&(DataLink_Status())&&(SleepState==0)&&(VocREC.running==0)) //  心跳
            	{
            ...
            // 
            if(1==SD_ACKflag.f_BD_DeviceAttribute_8107)
            	{
	     ...
	     //----------   远程下载相关 ------------		
			...
			// {	
             ...
             if((RdCycle_RdytoSD==ReadCycle_status)&&(0==ISP_running_state)&&(DataLink_Status()==1)&&(DEV_Login.Operate_enable==2)&&(VocREC.running==0))    // 读取发送--------- 正常GPS 
	 ...
	 if(UDP_dataPacket_flag==0x02 )
	 {
		 ...
		 // --------  sp 当前是0.1 knot------------------	 
		  ...
		  sp_DISP=sp/100;   //  sp_Disp 单位是 0.1km/h 
	         ...
	         //------------------------------ 通过GPS模块数据获取到的速度 --------------------------------
              if((0==Jia_GPS_spdFlag) ||(2==Jia_GPS_spdFlag))      //  不是加速度时用GPS 速度
			 Speed_gps=(u16)sp_DISP; 
		 else
		 	 Speed_gps_jiashi=(u16)sp_DISP;
	         //---------------------------------------------------------------------------
...
// 1. Head
 ...
 // 2. content 
	...
	memcpy(Original_info+Original_info_Wr,"Tianjin TCB TW705-BD",20);     
    Original_info_Wr+=20;      
    //  终端ID   7 Bytes    ,    
...
u8  Stuff_DeviceISP_BD_0108H(void)
{
   // 1. Head
...
u8  Stuff_DeviceISP_BD_0704H(void)
{
   // 1. Head
...
u8  Stuff_DeviceISP_BD_0705H(void)
{
   // 1. Head
...
u8  Stuff_DeviceISP_BD_0805H(void)
{
   // 1. Head
	...
	//  rt_kprintf("\r\n  1--- pic_total_num:  %d	current_num:  %d  MediaObj.Media_Type: %d \r\n ",MediaObj.Media_totalPacketNum,MediaObj.Media_currentPacketNum,MediaObj.Media_Type);
	 ...
	 if(MediaObj.Media_currentPacketNum==1)
	 {
	   ...
	   //  MediaCode Type 
		...
		Original_info[Original_info_Wr++]=((Gps_Gprs.Time[2]/10)<<4)+(Gps_Gprs.Time[2]%10);	 
#endif
	   if((1==MediaObj.Media_Type)&&(WriteMP3.Trans_mp3==0))  // 音频上传时需要增加头,mp3 不加头 
	   {
						...
						if(mp3_fsize>inadd)
						  {
						   ...
						   return false;
						DF_ReadFlash(DF_MP3_offset+Sound_sdState.SD_packetNum,0,Original_info + Original_info_Wr,readsize);
					     //  Original_info_Wr+=readsize;//	  
                      
                                   rt_kprintf("\r\n MP3 Sound_sdState.SD_packetNum= %d   filesize=%d  readsize=%d  \r\n",Sound_sdState.SD_packetNum,mp3_fsize,mp3_fsize-inadd);    
...
{  
    u8 i=0;
	 ...
	 // 2. content     old
		...
		memcpy(Original_info+Original_info_Wr,JT808Conf_struct.Driver_Info.Driver_ID,20); //  从业资格证编码
		Original_info_Wr+=20;
		memcpy(Original_info+Original_info_Wr,JT808Conf_struct.Driver_Info.Drv_CareerID,40); //  从业资格证编码
		Original_info_Wr+=40;
...
{  
  ...
  //---------------------------------------------
   ...
   if(!Protocol_Head(MSG_0x0900,Packet_Normal)) 
	...
	// 2. content	
	//	应答流水号
   Original_info[Original_info_Wr++]=0xFF;// 返回透传数据的类型 1	表示远程下载
	// 3. 内容
		//	3.1. Sub Head  
  ...
  switch(SubID)
   {
	 ...
	 case 0x0013:  //  主服务器地址  IP 或域名
					...
					i =str2ip((char*)reg_in, RemoteIP_main_reg); 
					 if (i <= 3)
					 {
					  rt_kprintf("\r\n  域名: %s \r\n",reg_in); 
					 ...
					 Api_Config_write(config,ID_CONF_SYS,(u8*)&SysConf_struct,sizeof(SysConf_struct));
					  ...
					  break;
				        ...
				        //   假的
					 memset(reg_str,0,sizeof(reg_str));
					  IP_Str((char*)reg_str, *( u32 * ) RemoteIP_main_reg);  	
					 strcat((char*)reg_str, " :");		 
  ...
  if((Sound_sdState.photo_sending==enable)&&(1==MediaObj.Media_Type)) // 音频
  {
	 if((Sound_sdState.SD_packetNum<=Sound_sdState.Total_packetNum+1)&&(2!=MediaObj.RSD_State))  
	  {  //  一下定时器在	在顺序发送过过程中	 和   收到重传开始后有效
		  ...
		  if( Sound_sdState.Data_SD_counter>20)         
			  {
	...
	if((Photo_sdState.photo_sending==enable)&&(0==MediaObj.Media_Type))
	{
	   if((Photo_sdState.SD_packetNum<=Photo_sdState.Total_packetNum+1)&&(2!=MediaObj.RSD_State))  
	   	{  //  一下定时器在   在顺序发送过过程中   和   收到重传开始后有效 
			...
			if( Photo_sdState.Data_SD_counter>20)  //40   12     
				{
		  ...
		  case  0xFF03:   //  扩展终端参数设置1 命令
			...
			//---------  BD  add -------------------------------
                     case  0x8003: // BD--8.4 分包请求上传
                                          Detach_PKG.Original_floatID=(UDP_HEX_Rx[13]<<24)+UDP_HEX_Rx[14];//原始消息流水号
									...
									if((Warn_Status[1]&0x20)&&(0x00002000&HumanConfirmWarn.ConfirmWarnType)) // 进出路线报警
									 ...
									 if((Warn_Status[1]&0x40)&&(0x00004000&HumanConfirmWarn.ConfirmWarnType))  // 路段行驶时间不足、过长
									 {
									   ...
									   if((Warn_Status[0]&0x10)&&(0x10000000&HumanConfirmWarn.ConfirmWarnType))  //车辆非法位移
									  {
									      ...
									      if((Warn_Status[0]&0x60)&&(0x60000000&HumanConfirmWarn.ConfirmWarnType))  //碰撞侧翻报警
									  {
                                                                              Warn_Status[0]&=~0x60; // 2 个bit
									  } 
...
{
       if((GPS_speed>=300)&&(GPS_speed_BAK<300)) 
	  {  	   
	        if(GPS_ExtOnce==0)
	        {
	             PositionSD_Enable();
	   ...
	   if((GPS_speed>=150)&&(GPS_speed_BAK<150)&&(TiredConf_struct.Tired_drive.ACC_ONstate_counter))  	
	  	{
	  	      	        if(GPS_ExtOnce==0)
			        {
			             PositionSD_Enable();
	   ...
	   if(GPS_speed<=5)    // clear  once
          ...
          if((GPS_speed_BAK>=100)&&(GPS_speed<100))
	     {
	       //if(GPS_ExtOnce==0)
	        {
	             PositionSD_Enable();
...
{ 
     u32 Reg_Timer=0,delta_timer=0;
	 
     		 //  if(DispContent==2) 
			//  	rt_kprintf("\r\n				速度: %d Km/h    Debugspd=%d KM/h \r\n",GPS_speed/10,DebugSpd/10);	   
                  //------------------------------------------------------
                  if((Flag_tired==2)&&(TiredConf_struct.TiredDoor.Door_DrvKeepingSec==300))
                  {
                       // Reg_Timer=time_now.hour*3600+time_now.min*60+time_now.sec; 
			  Reg_Timer=( u32 ) ( CurrentTime[0] * 60 * 60 ) + ( u32 ) ( CurrentTime[1] * 60 ) + ( u32 )  CurrentTime[2];  
			  delta_timer=Reg_Timer-TiredStart_time; 	 		
			...
			if(GPS_speed>60)  // GPS_speed 单位为0.1 km/h  速度大于6 km/h  认为是行驶 
	         // if(DebugSpd>60) 
			  {   				      
				 //-------------疲劳驾驶相关 ----------------------- 
                          if(TiredConf_struct.Tired_drive.ACC_ONstate_counter==0)
				{	
				     rt_kprintf("\r\n疲劳 start %d-%d-%d %02d:%02d:%02d\r\n", time_now.year+2000, time_now.month, time_now.day, \
                                    // time_now.hour,time_now.min,time_now.sec);
					   CurrentTime[0] , CurrentTime[1] , CurrentTime[2] );   
			            //---- 触发即时上报数据-------------------------
							PositionSD_Enable();    
					               Current_UDP_sd=1;	
					if(Flag_tired==1)			   
				      	    {    TiredStart_time=( u32 ) ( CurrentTime[0] * 60 * 60 ) + ( u32 ) ( CurrentTime[1] * 60 ) + ( u32 )  CurrentTime[2];Flag_tired=2;} 
		  	            //-----------------------------------------------------------
                          	}
...
{    
    memset(JT808Conf_struct.Vechicle_Info.Vech_Num,0,sizeof(JT808Conf_struct.Vechicle_Info.Vech_Num));
    memcpy(JT808Conf_struct.Vechicle_Info.Vech_Num,instr,7); 
    Api_Config_Recwrite_Large(jt808,0,(u8*)&JT808Conf_struct,sizeof(JT808Conf_struct)); 
}
...
FINSH_FUNCTION_EXPORT(current_pl, current_pl);   
...
void tiredon(void)
{
	...
	PositionSD_Enable();    
      Current_UDP_sd=1;	
	//-------------------------------------	   
      rt_kprintf("\r\n疲劳 end %d-%d-%d %02d:%02d:%02d\r\n", time_now.year+2000, time_now.month, time_now.day, \
                                 //    time_now.hour,time_now.min,time_now.sec);
					   CurrentTime[0] , CurrentTime[1] , CurrentTime[2] );      


}
FINSH_FUNCTION_EXPORT(tiredon, tiredon);   

void play_tiredon()
{
      TTS_play("您已经疲劳驾驶，请注意休息");   
}
FINSH_FUNCTION_EXPORT(play_tiredon, play_tiredon);   

void play_tiredoff(void)
{
    TTS_play("您的疲劳驾驶报警已经解除"); 
}
FINSH_FUNCTION_EXPORT(play_tiredoff, play_tiredoff);   
...
void tiredoff(void)
{
   ...
   PositionSD_Enable();    
   Current_UDP_sd=1;	 
   rt_kprintf("\r\n疲劳 clear %d-%d-%d %02d:%02d:%02d\r\n", time_now.year+2000, time_now.month, time_now.day, \
                                   //  time_now.hour,time_now.min,time_now.sec);
					  CurrentTime[0] , CurrentTime[1] , CurrentTime[2] );   

}
FINSH_FUNCTION_EXPORT(tiredoff, tiredoff);    

void gpsspd(u8 *strin)   //  GPS 用假速度
{
   sscanf(strin, "%d", (u32*)&Speed_gps);
   rt_kprintf("\r\n set gpsspd= %d s\r\n",Speed_gps); 
...
void auto_tired(u8 *instr)
{
     if(instr[0]=='1')
      {     Flag_tired=1;  rt_kprintf("\r\n auto_tired enable\r\n");  }
     else	 
        {   Flag_tired=0;  rt_kprintf("\r\n auto_tired disable\r\n");   }
}
FINSH_FUNCTION_EXPORT(auto_tired, auto_tired);     


void sensorspd(u8 *strin)   //  GPS 用假速度
{
   sscanf(strin, "%d", (u32*)&Speed_cacu);
   rt_kprintf("\r\n set sensorspd= %d s\r\n",Speed_cacu);  

  //  JT808Conf_struct.Speed_GetType=1;  // 通过速度传感器 获取速度
    Jia_GPS_spdFlag=2;  
}
FINSH_FUNCTION_EXPORT(sensorspd, sensorspd);     
